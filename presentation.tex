...
\vspace{-5mm}
    \begin{columns}[t]
        \begin{column}{0.55\textwidth}
        \small
        \setlist{label=$\diamond$}
        \begin{itemize}
            \item<1-> Toolkit adopted by the ATLAS collaboration for the validation of their software installation
            \item<2-> Fast benchmark candidate using the GEANT4 simulation of the ATLAS detector
            \item<3-> Simulates $n$ muon particles propagating through the ATLAS
        ...
        \begin{figure}[H]
        \begin{center}
            \includegraphics[scale=0.35]<4->{images/dual-mode-gaussian.png}
        \end{center}
        \end{figure}
